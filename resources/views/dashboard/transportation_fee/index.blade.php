...
@section('content')
    <div class="container">
        <div class="d-flex justify-content-between align-items-center mb-5">
            ...
            <a href="{{ route('transportation_part.edit', $student_transportation->id) }}" class="btn btn-warning btn-sm">تعديل الاقساط</a>
        </div>
                ...
                @foreach ($parts as $part)
                    <div class="col-6">
                        <ul class="list-group list-group-horizontal border-0 w-100 rounded-0">
                            ...
                            <li class="list-group-item border-0 border-bottom bg-white col-6 col-md-4 rounded-0">
                                {{ $part->amount }}</li>
                        </ul>
                    </div>
                @endforeach
        ...
        <section class="mt-4">
            @foreach ($parts as $part)
                @if ($part->paid == 0)
                    <ul class="list-group list-group-horizontal border-0 w-100 rounded-0">
                        <li
                            class="list-group-item border-0 border-bottom bg-primary col-3 rounded-0 text-white d-flex align-items-center">
                            دفع القسط ({{ $loop->iteration }})
                        </li>
                        <li
                            class="list-group-item border-0 border-bottom col-9 d-flex align-items-center justify-content-between">
                            <span class="d-flex align-items-center">{{ number_format($part->amount) }}</span>
                            <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                data-bs-target="#paymentModal_{{ $part->id }}">دفع</button>
                        </li>
                    </ul>

                    <!-- Modal -->
                    <div class="modal fade" id="paymentModal_{{ $part->id }}" tabindex="-1"
                        aria-labelledby="paymentModal_{{ $part->id }}Label" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body" x-data="{ payment_type: 0 }">
                                    <form action="{{ route('transportation_part.update', $part->id) }}" method="POST"
                                        ...
                                        <template x-if="payment_type == 2">
                                            <div class="mb-3">
                                                <label for="attachment" class="form-label">الاشعار</label>
                                        ...
                                        <template x-if="payment_type == 3">
                                            <div class="mb-3">
                                                ...
                                                <input type="number" name="check_number" id="check_number"
                                                    class="form-control" required>
                                            </div>
                                        </template>
                                        ...
                                        <template x-if="payment_type == 3">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" id="check_owner" id="check_owner" name="check_owner"
                                                    class="form-control" required>
                                            </div>
                                        </template>
            ...
            @foreach ($parts as $part)
                @if ($part->paid == 1)
                    <ul class="list-group list-group-horizontal border-0 w-100 rounded-0">
                        <li
                            class="list-group-item border-0 border-bottom bg-primary col-3 rounded-0 text-white d-flex align-items-center">
                            القسط ({{ $loop->iteration }})
                        </li>
                        <li
                            class="list-group-item border-0 border-bottom col-9 d-flex justify-content-between align-items-center bg-success text-white">
                            <strong class="d-lfex">{{ number_format($part->amount) }}</strong>
                            <aside>
                                <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                    data-bs-target="#receiptModal_{{ $part->id }}">عرض</button>
                                <a href="{{ route('part.receipt', $part->id) }}" target="_blank"
                                    class="btn btn-primary">الايصال</a>
                            </aside>
                        </li>
                    </ul>

                    <!-- Modal -->
                    <div class="modal fade" id="receiptModal_{{ $part->id }}" tabindex="-1"
                        aria-labelledby="receiptModal_{{ $part->id }}Label" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                ...
                                <div class="modal-body">
                                    ...
                                    @if ($part->payment_type == 2)
                                        ...
                                        <img src="{{ asset('images/transportation_payment/' . $part->attachment) }}" alt=""
